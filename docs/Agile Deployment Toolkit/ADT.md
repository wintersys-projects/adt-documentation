...
There is one additional "server type agnostic" repository which if you are interested in extending the toolkit to supoort additional server types to those listed above such as caching servers or email servers you can use this repository as a template upon which to customise for the particular class of server you are interested supporting (in other words, customising with automated mailcow installation or iredmail or plesk installation if want an email server type machine or a redis or memcached installation if you want a caching server) and so on. 

**[Agile Infrastructure Template Scripts]( https://github.com/wintersys-projects/adt-default-template-scripts)**
