...
There is one additional repository type which if you are interested in extending the toolkit to supoort additional server types to those listed above such as caching servers or email servers you can use this repository as a template upon which to customise for the particular class of server you are interested supporting (in other words, customising with automated mailcow installation or iredmail installation if want an email server type machine) and so on. 
