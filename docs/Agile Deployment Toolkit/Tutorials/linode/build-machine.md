...
2) Choose the first option here if you are using an expedited style build and the second option if you are deploying a hardcore type build and if you have come here from the QuickDemos, take the third option to deploy the demos using a user-data approach. 
...
- **QUICK DEMO BUILD STYLE**: Get copy of [this](https://github.com/wintersys-projects/adt-build-machine-scripts/blob/main/templatedconfigurations/quickdemo/templates/linode/linode.tmpl). Modify the variables marked "CUSTOMISE ME" to be specific to you and then follow [this] to find out the mimimum set of variables you need to change for the demo you want to deploy.
